...
Tttalk.prototype = {
  ...
  savePhoto : function(from_lang, to_lang, filename, openid, callback) {
    ...
    oss.putObject(sourceFile, dest, 'image/jpeg', 0, function(err, data) {
      message_dao.saveMessage(from_lang, to_lang, 'photo', filename, openid, function(err, newId){
        callback(err, newId);
  ...
  saveVoice : function(from_lang, to_lang, filename, openid, callback) {
    ...
    oss.putObject(sourceFile, dest, 'audio/amr', 0, function(err, data) {
      message_dao.saveMessage(from_lang, to_lang, 'voice', filename, openid, function(err, newId){
        callback(err, newId);
  ...
  saveText : function(from_lang, to_lang, content, openid, callback) {
      message_dao.saveMessage(from_lang, to_lang, 'text', content, openid, callback);
  },
    ...
    message_dao.findByOpenid(openid, function(err, results) {
      for(var i in results) {
        ...
        if(result.filetype == 'photo')
          result.from_content = '图片翻译';
        else if(result.filetype == 'voice')
          result.from_content = '语音翻译';
        result.create_date = moment(result.create_date).format("YYYY-MM-DD HH:mm:ss")
      }
      callback(err, results);
    });
  }
