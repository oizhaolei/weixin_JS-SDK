...
Tttalk.prototype = {
  ...
  // }
  _charge : function(openid, fee, transaction_id, out_trade_no, bank_type, fee_type, time_end, trade_type, memo, callback) {
    ...
    if (wxmessage.result_code === 'SUCCESS' && wxmessage.return_code === 'SUCCESS') {
      charge_dao.getByTransactionId(wxmessage.transaction_id, function(err, charge) {
        if (charge) {
          ...
          self._charge(openid, wxmessage.total_fee, wxmessage.transaction_id, wxmessage.out_trade_no, wxmessage.bank_type, wxmessage.fee_type, wxmessage.time_end, wxmessage.trade_type, wxmessage.memo, callback);
        }
      });
    } else {
      callback('交易未成功，无法充值。');
  ...
  change_account : function(openid, username, sex, callback) {
    account_dao.updateAccount(openid, {
      username : username,
      sex : sex
    }, function(err, results, account) {
