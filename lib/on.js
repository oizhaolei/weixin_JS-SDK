...
On.prototype = {
  ...
  onShareToFriend : function(openid) {
    // send text first
    var share_msg = i18n.__('share_to_friend_msg', parseFloat(config.subscribe_reward) / 100);
    wxservice.text(openid, share_msg, function(err, data) {
    });
    ...
    nwLink.qrcode.permanent.createString(app, openid, function (err, json) {
      if (err) {
        ...
        var qr = path.join(config.tmpdir,  openid + '.jpg');
        var qr_file = fs.createWriteStream(qr);
        request(qr_url).pipe(qr_file);

        qr_file.on('finish', function() {
          logger.info('curl finish: %s', qr);
          //write a post image
          var output = path.join(config.tmpdir,  openid + '_output.jpg');
          var background = path.join(__dirname, "post_background.png");
          var images = require("images");
          images(background)
            .draw(images(qr).size(200), 10, 10)
            .save(output, {
              quality : 50
            });
