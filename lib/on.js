...
On.prototype = {
  ...
  onSign : function(openid) {
    // 用户 签到
    account_dao.checkSignAccount(openid, function(err, sign) {
      if (err) {
        logger.info(err);
      } else {
        if(sign == null){
          //今天没有签到，签到
          account_dao.insertSignAccount(openid, function(err, results) {
            if (err) {
              logger.info(err);
            } else {
              var to_content = i18n.__('sign_ok');
              //随机数中奖计算
              var signRandom = new Number(Math.random() * config.sin_random).toFixed(0);
              logger.info('sign_random: %s', signRandom);
              if(signRandom == 1){
                //给用户奖励
                account_dao.updateAccount(openid, {fee : config.sin_money}, function(err, results, account) {
                  logger.debug('sign_random %s', openid);
                });
                to_content += "\n\n -- " + i18n.__('sign_pay', parseFloat(config.sin_money) / 100);
              } else {
                to_content += "\n\n -- " + i18n.__('sign_no_pay');
              } 
          ...
          //今天已经签过了
          var to_content = i18n.__('sign_done');
          wxservice.text(openid, to_content, function(err, data) {
          });
        }
      }
